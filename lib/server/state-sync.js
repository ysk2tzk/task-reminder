...
export async function readState() {
  ...
  const [{ data: tasks, error: taskError }, { data: occurrences, error: occurrenceError }, { data: settings, error: settingsError }] = await Promise.all([
    ...
    supabase.from("task_occurrences").select("*, tasks(title, description)").eq("status", "pending").lte("scheduled_at", todayEndIso),
...
export async function readHistory({ page = 1, pageSize = 10, query = "" } = {}) {
  const supabase = getSupabaseAdmin();
  const safePage = Math.max(1, Number(page) || 1);
  const safePageSize = Math.min(10, Math.max(1, Number(pageSize) || 10));
  let request = supabase
    .from("task_occurrences")
    .select("*, tasks!inner(title, description)", { count: "exact" })
    .in("status", ["completed", "skipped"])
    .order("updated_at", { ascending: false })
    .range((safePage - 1) * safePageSize, safePage * safePageSize - 1);
  if (query.trim()) {
    request = request.ilike("tasks.title", `%${query.trim()}%`);
  }
  const { data, count, error } = await request;
  if (error) {
    throw createHttpError(500, error.message);
  }
  return { occurrences: (data || []).map(deserializeOccurrence), totalCount: count || 0 };
}
