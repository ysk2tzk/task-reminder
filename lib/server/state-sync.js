...
export function normalizeTask(task) {
  return {
    ...
    monthly_day: task.scheduleType === "monthly" ? task.monthlyDay : null,
...
export async function syncState(payload) {
  ...
  if (taskError) {
    throw createHttpError(500, translateTaskWriteError(taskError, taskRows));
  }
...
function translateTaskWriteError(taskError, taskRows) {
  const rawMessage = taskError?.message || "tasks の保存に失敗しました。";
  const includesMonthlyTask = taskRows.some((row) => row.schedule_type === "monthly");

  if (
    includesMonthlyTask
    && rawMessage.includes("tasks_schedule_consistency_check")
  ) {
    return "月次タスクを保存できませんでした。Supabase の tasks 制約が古い可能性があります。最新の `実行支援_DDL.sql` を再実行して、`tasks_schedule_consistency_check` に monthly 条件が含まれていることを確認してください。";
  }

  if (
    includesMonthlyTask
    && (rawMessage.includes("monthly_day") || rawMessage.includes("schedule_type"))
  ) {
    return "月次タスクを保存できませんでした。Supabase の tasks テーブル定義が古い可能性があります。最新の `実行支援_DDL.sql` を適用してください。";
  }
...
function isTerminalOccurrence(row) {
  return row.status === "completed" || row.status === "skipped" || row.status === "expired";
}
...
export async function ensurePendingOccurrences(supabase = getSupabaseAdmin()) {
  const { data: tasks, error } = await supabase
    .from("tasks")
    .select("id, schedule_type, scheduled_date, monthly_day, scheduled_time, weekdays, is_active")
    .eq("is_active", true);
...
function generateScheduledAtValues(task, startDateKey, endDateKey) {
  ...
  for (const dateKey of iterateDateKeys(startDateKey, endDateKey)) {
    ...
    if (task.schedule_type === "monthly") {
      shouldInclude = getDayFromDateKey(dateKey) === getMonthlyScheduledDay(dateKey, task.monthly_day);
    }
...
function getMonthlyScheduledDay(dateKey, monthlyDay) {
  const [year, month] = dateKey.split("-").map(Number);
  const lastDay = new Date(Date.UTC(year, month, 0)).getUTCDate();
  return Math.min(Number(monthlyDay), lastDay);
}
...
export async function cleanupDuplicateOccurrences(supabase = getSupabaseAdmin()) {
  const { data, error } = await supabase
    .from("task_occurrences")
    .select("id, task_id, scheduled_at, status, completed_at, skipped_at, expired_at, next_notification_at, updated_at, created_at");
...
function compareOccurrenceRowsForDedup(left, right) {
  ...
  const terminalTime = (row) => row.completed_at || row.skipped_at || row.expired_at || row.next_notification_at || row.updated_at || row.created_at || "";
