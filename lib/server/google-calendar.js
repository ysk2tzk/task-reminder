...
const GOOGLE_EVENT_TIME_ZONE = "Asia/Tokyo";
...
export async function updateGoogleCalendarTarget(calendarId) {
  if (!calendarId) {
    throw createHttpError(400, "登録先カレンダーを選択してください。");
  }

  const settings = await getStoredGoogleCalendarSettings();
  const calendars = await listWritableGoogleCalendars();
  if (!calendars.some((calendar) => calendar.id === calendarId)) {
    throw createHttpError(400, "選択したカレンダーへの書き込み権限がありません。");
  }
...
function buildCalendarEvent({ summary, description, startedAt }) {
  ...
  return {
    ...
    description,
    start: {
      dateTime: formatGoogleEventDateTime(startedAt),
      timeZone: GOOGLE_EVENT_TIME_ZONE,
    },
    end: {
      dateTime: formatGoogleEventDateTime(endedAt),
      timeZone: GOOGLE_EVENT_TIME_ZONE,
    },
  };
}
...
function formatGoogleEventDate(value) {
  return value.toLocaleString("ja-JP", { timeZone: GOOGLE_EVENT_TIME_ZONE });
}

function formatGoogleEventDateTime(value) {
